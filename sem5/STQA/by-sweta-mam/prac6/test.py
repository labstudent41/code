...
def test():
    sub1highestmarks={'student':"","marks":0}
    sub2highestmarks={'student':"","marks":0}
    sub3highestmarks={'student':"","marks":0}
    ...
    for row in rows[1:]:
        ...
        if int(cell[3].text)>=60 or int(cell[4].text)>=60 or int(cell[5].text)>=60:
            students_with_60_marks.append([cell[1].text,cell[2].text,int(cell[3].text),int(cell[4].text),int(cell[5].text)])
